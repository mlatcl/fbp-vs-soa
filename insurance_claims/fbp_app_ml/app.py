...
class PredictClaimComplexity(INode):
    def __init__(self, training_artifacts_dir_path, **kwargs):
        super(PredictClaimComplexity, self).__init__(**kwargs)
        InputPlug('claims', self)
        OutputPlug('simple_claims', self)
        OutputPlug('complex_claims', self)

        self.training_artifacts_dir_path = training_artifacts_dir_path

        model_path = self.training_artifacts_dir_path.joinpath("fbp_model.obj")
    ...
    def compute(self, claims: List[Dict]) -> Dict:
        ...
        for column_name in df.columns:
            if df[column_name].dtype == object:
                ...
                file_name = self.training_artifacts_dir_path.joinpath(f"{column_name}_encoder.obj")
...
class App():
    def __init__(self, training_artifacts_dir_path):
        self.training_artifacts_dir_path = training_artifacts_dir_path
        self._build()
    ...
    def _build(self) -> Graph:
        ...
        predict_claim_complexity = PredictClaimComplexity(self.training_artifacts_dir_path, graph=graph)
        self.new_claims_stream.outputs["new_claims"] >> predict_claim_complexity.inputs["claims"]

...
applications = data["App"].tolist()
applications = list(dict.fromkeys(applications))
metrics.remove("Key")
metrics.remove("App")
for metric in metrics:
    for application in applications:
        data_fbp = data.loc[(data["Key"].isin(["fbp_app_min", "fbp_app_data", "fbp_app_ml"])) & (data["App"] == application)]
        data_soa = data.loc[(data["Key"].isin(["soa_app_min", "soa_app_data", "soa_app_ml"])) & (data["App"] == application)]
        X = ["Min", "Data", "ML"]
        fig, ax = plt.subplots()
        ...
        norm = np.sqrt(u**2+v**2)
        ax.quiver(pos_x, pos_y, u/norm, v/norm, angles="xy", zorder=5, pivot="mid")
        ax.set(xlabel="Application Stage", ylabel=metric)
        ax.grid(linestyle="-", linewidth="0.25", color="grey")
        ax.legend(title="Application Type", fancybox=True)
        plt.savefig("figs-conference/"+application+"_"+metric.replace(" ", "_").lower()+".png")

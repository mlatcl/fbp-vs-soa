...
metrics.remove("App Key")
for metric in metrics:
    data_fbp = data.loc[data["App Key"].isin(["fbp_app_min", "fbp_app_data", "fbp_app_ml"])]
    data_soa = data.loc[data["App Key"].isin(["soa_app_min", "soa_app_data", "soa_app_ml"])]
    X = ["Min", "Data", "ML"]
    fig, ax = plt.subplots()
    ...
    plt.savefig("figs/"+metric.replace(" ", "_").lower()+".pdf")
